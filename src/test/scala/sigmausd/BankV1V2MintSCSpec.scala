...
class BankV1V2MintSCSpec
  ...
  val stableCoinCirculationIn   = 1_000L
  val reserveCoinBankIn         = stableCoinBankIn * 5
  val reserveCoinBankOut        = reserveCoinBankIn
  val reserveCoinCirculationIn  = stableCoinCirculationIn * 5
  val reserveCoinCirculationOut = reserveCoinCirculationIn
  val oracleRateXy              = 1L * NANO     // x/100 nanoErgs per US Cent
  val bankBoxNanoErgsInGen      = Gen.const(1_000L * NANO)

  def bankV1SCMintingTx(
  ...
  def bankV2SCMintingTx(
    ...
    val updatedLimits =
      (limit - bankReserveDeltaAfterFee, limit) // sc exchange
  ...
  property("Small amount minting V1 should work") {
    createMockedErgoClient().execute { implicit ctx: BlockchainContext =>
      forAll(
        ...
        assertTry(bankV1SCMintingTx(bankBoxNanoErgsIn, exchangeDeltaSmall))
      }
    }
  }
  ...
  property("Small amount minting V2 should work") {
    createMockedErgoClient().execute { implicit ctx: BlockchainContext =>
      forAll(
        ...
        assertTry(bankV2SCMintingTx(bankBoxNanoErgsIn, exchangeDeltaSmall))
      }
    }
  }

  /* BIG MINTING STABLE COINS */
  val exchangeDeltaBig = 491L // 490L passes for v2
  ...
  property(">0.5% amount minting V1 should work") {
    createMockedErgoClient().execute { implicit ctx: BlockchainContext =>
      forAll(
        ...
        assertTry(bankV1SCMintingTx(bankBoxNanoErgsIn, exchangeDeltaBig))
      }
    }
  }
  ...
  property(">0.5% amount minting V2 should fail") {
    createMockedErgoClient().execute { implicit ctx: BlockchainContext =>
      forAll(
        ...
        assertTryNeg(bankV2SCMintingTx(bankBoxNanoErgsIn, exchangeDeltaBig))
      }
    }
  }
}
